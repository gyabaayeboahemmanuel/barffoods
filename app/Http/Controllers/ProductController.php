        ...
        try {
            ...
            return response()->json([
                ...
                'categories' => Category::active()->ordered()->limit(50)->get()->map(function ($category) {
                    $firstProduct = $category->products()->where('is_active', true)->select('image', 'images')->first();
                    $imageUrl = null;
                    if ($firstProduct) {
                        $images = $firstProduct->images ?? [];
                        $imageUrl = !empty($images) ? $images[0] : ($firstProduct->image ?? null);
                    }
                    return [
                        'id' => (string) $category->id,
                        'name' => $category->name,
                        'product_count' => $category->products()->where('is_active', true)->count(),
                        'first_product_image' => $imageUrl,
                    ];
                })
