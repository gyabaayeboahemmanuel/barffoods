...
export function CartProvider({ children, user }: CartProviderProps) {
    ...
    // When user is set, always use authenticated cart so we don't mix with anonymous
    ...
    const fetchCart = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // When logged in, don't overwrite with anonymous cart (token may not be ready yet)
                if (user) {
                    // Leave cart state unchanged; token will be used on next fetch after fetchSanctumToken
                    return;
                }
                // Guest: clear stored token and try anonymous cart
    ...
    const refreshCartWithEndpoint = async (endpoint: string) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // When logged in, don't overwrite cart with anonymous (would hide just-added items)
                if (!user) {
                    localStorage.removeItem('sanctum_token');
                    try {
                        const anonymousResponse = await fetch('/api/cart/anonymous', {
    ...
    useEffect(() => {
        if (user) {
            // Ensure we have the API token before fetching cart so /api/cart returns 200
            fetchSanctumToken().then(() => fetchCart());
        } else {
            localStorage.removeItem('sanctum_token');
            fetchCart();
        }
